...
struct Args {
    ...
    /// Include only files matching these glob patterns (comma-separated or repeated)
    #[arg(short, long, value_delimiter = ',')]
    include: Vec<String>,

    /// Exclude files matching these glob patterns (comma-separated or repeated)
    #[arg(short, long, value_delimiter = ',')]
    exclude: Vec<String>,
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Determine diff source for initial fetch and live-reload
    let diff_source = if let Some(ref diff_args) = args.diff_args {
        diff::DiffSource::WithArgs(diff_args.clone())
    } else if !args.from.is_empty() && !args.to.is_empty() {
        diff::DiffSource::Between(args.from.clone(), args.to.clone())
    ...
    let file_filter = diff::FileFilter::new(&args.include, &args.exclude)?;
    let (mut file_changes, left_label, right_label) = diff::refresh_diff(&diff_source)?;
    file_filter.apply(&mut file_changes);
